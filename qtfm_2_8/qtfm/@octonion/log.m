...
if isreal(X)
    ...
    % X is a real octonion, and we compute the logarithm of an isomorphic
    % complex number using the standard Matlab log function, then construct
    % an octonion with the same axis as the original octonion.
    
    Y = isooctonion(log(isocomplex(X)), X);
else
    
    % X is a complex octonion (or at least some elements of X are), and
    % therefore we cannot use the method above for real octonions,
    ...
    Y = octonion(log(normq(X))./2, axis(X) .* angle(X));
...
% real or complex value, and the second is a pure octonion (real
% or complex). Therefore we can compose them using the octonion
% constructor function.

% $Id: log.m 1053 2020-02-17 18:13:03Z sangwine $
